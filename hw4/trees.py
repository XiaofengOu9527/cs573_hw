...
# =============================================================================
	def selectAttri(self, target_attri, flag='all', num_attri=0):
		if flag == 'all' or num_attri > len(self.attributes):
			attributes = self.attributes
		elif flag == 'part':
			attributes = random.sample(self.attributes, num_attri)
		...
		for idx in self.indices:
			...
			for i in range(n):
				split_value = int(row[attributes[i]])       
		...
		attri = attributes[int(np.argmin(gini_index))]
		self.name = attri
...
# =============================================================================
	...
	def split(self, target_attri, depth_limit=8, example_limit=50, flag='all', num_attri=0):
		...
		# non-leaf node, split
		attri = self.selectAttri(target_attri, flag=flag, num_attri=num_attri)
		...
		children_attris = self.attributes
...
def decisionTree(trainingSet, testSet, depth_limit=8, example_limit=50, flag='all', num_attri=0):
	"""
	num_attri: number of attributes to be chosen to split at each node
	"""

	all_attris = trainingSet.columns.values.tolist()
	tree_attris = [attri for attri in all_attris if attri not in ['decision']]
	...
	while len(nodes_to_split) > 0:
		...
		attri, left_child, right_child = curr_node.split('decision', depth_limit=depth_limit, example_limit=example_limit, flag=flag, num_attri=num_attri)
...
def bagging(trainingSet, testSet, num_tree=30, depth_limit=8, example_limit=50):
	...
	for i in range(num_tree):
		...
		trees[i], train_predict, test_predict, _, _ = decisionTree(trainingSet=data, testSet=testSet, depth_limit=depth_limit, example_limit=example_limit, flag='all', num_attri=0)
...
def randomForest(trainingSet, testSet, num_tree=30, depth_limit=8, example_limit=50):
	...
	all_attris = trainingSet.columns.values.tolist()
	tree_attris = [attri for attri in all_attris if attri not in ['decision']]
	# number of attris to chose at each node
	num_attri = math.floor(math.sqrt(len(tree_attris))) 

	for i in range(num_tree):
		data = trainingSet.sample(frac=1, replace=True)		# bootstrap sampling	
		trees[i], train_predict, test_predict, _, _ = decisionTree(trainingSet=data, testSet=testSet, depth_limit=depth_limit, example_limit=example_limit, flag='part', num_attri=num_attri)	
